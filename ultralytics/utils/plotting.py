...
class Annotator:
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        try:
            ...
            # 如果utils模块不存在，添加内联函数
            def pixels_to_cm(pixel_value, use_adaptive=False):
                return pixel_value / Config.pixels_per_mm / 10
        ...
        # 计算中心点坐标，用于截点和坏花类别显示
        if rotated:
            center_x, center_y = np.mean(np.array(box)[:, 0]), np.mean(np.array(box)[:, 1])
        else:
            center_x, center_y = (box[0] + box[2]) / 2, (box[1] + box[3]) / 2
            
        # 检查标签中是否包含"截点"或"坏花"文字，如果是则添加中心点坐标（转换为厘米）
        is_cut_point = "截点" in label or "cut_point" in label
        is_bad_flower = "坏花" in label or "bad_flower" in label
        
        # 从标签中移除置信度值（如果有）
        if " " in label:
            label = label.split(" ")[0]  # 只保留类别名称，去掉后面的置信度值
        
        if is_cut_point or is_bad_flower:
            # 将像素坐标转换为厘米单位（以左上角为原点），使用自适应比例
            try:
                cm_x = utils.pixels_to_cm(center_x, use_adaptive=True)
                cm_y = utils.pixels_to_cm(center_y, use_adaptive=True)
            except:
                cm_x = pixels_to_cm(center_x) * adaptive_scale_factor
                cm_y = pixels_to_cm(center_y) * adaptive_scale_factor
                
            # 只显示坐标数字，不显示其他信息
            coord_text = f"({cm_x:.2f},{cm_y:.2f})"
            label = f"{label} {coord_text}"
        ...
        if self.pil or not is_ascii(label):
            ...
            if label:
                ...
                if is_cut_point or is_bad_flower:
                    ...
                    p1 = (p1[0], box[3])
                self.draw.rectangle(
                    (p1[0], p1[1], p1[0] + w + 1, p1[1] + h + 1),
                    fill=color,
                )
                self.draw.text((p1[0], p1[1]), label, fill=txt_color, font=self.font)
        else:  # cv2
            if rotated:
